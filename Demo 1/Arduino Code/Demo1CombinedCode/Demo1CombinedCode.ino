...
void PID_CONTROL(){
    ...
    double phi_dot_des = phi_er * Kp + phi_integral * Ki * 0.002;


    //Find how far we are from desired location.

    if((abs(rho_s - rho) > 0.1) && (phi_er < PI/128)){
     if(rho_s - rho >0){
      rho_dot_des = 20;
     } else{
      rho_dot_des = -20;
    ...
    rho = rho + rho_dot_curr*0.0002;
     



    while( innerLoopTime + micros() < 200); 
    //End Inner Loop
    ...
    while(outerLoopTime + micros() <  1000);
    //End Outer Loop 
}
